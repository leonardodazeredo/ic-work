...
def split_data():
    bananadata = pd.read_csv("./hw3/tarefa2/banana.csv")
    bananadata_train, bananadata_test = train_test_split(bananadata, test_size=0.20)

    bananadata_train.to_csv("./hw3/tarefa2/bananadata_train.csv", sep='\t')
    bananadata_test.to_csv("./hw3/tarefa2/bananadata_test.csv", sep='\t')


if __name__ == '__main__':
    split_data()
else:
    bananadata_train = pd.read_csv("./hw3/tarefa2/bananadata_train.csv", sep='\t').drop('I', axis=1)
    print(bananadata_train.shape)
    X_train = bananadata_train.drop('Class', axis=1)
    y_train = bananadata_train['Class']

    bananadata_test = pd.read_csv("./hw3/tarefa2/bananadata_test.csv", sep='\t').drop('I', axis=1)
